...
PROPOSAL_DEPARTMENT_LABELS = {
    ...
    "hr": "HR Executive",
    "operations": "Technical Architect Agent",
    "ceo": "CEO",
}
    ...
    with engine.begin() as db:
        ...
        db.execute(text("""
            CREATE TABLE IF NOT EXISTS weekly_project_updates (
                ...
                generated_by TEXT DEFAULT 'Technical Architect Agent',
                generated_at TEXT,
                published_by TEXT,
...
def save_client_response(proposal_id, client_account, response, comment):
    ...
    if response == "Accept Proposal":
        return True, "Your proposal acceptance was saved. Please submit the detailed requirement next; the Technical Architect Agent will generate the delivery plan and weekly targets."
...
def ensure_project_for_accepted_proposal(report, client_account):
    ...
    row = {
        ...
        "operations_owner_id": "",
        "operations_owner_name": "Technical Architect Agent",
        "kickoff_date": "",
        "created_at": now,
        "updated_at": now,
    }
...
7. Roles Needed
- Technical Architect Agent / Project Owner
- Backend Developer
- Frontend Developer
- AI/ML Engineer
- QA Engineer
...
"""
    ...
    prompt = f"""
You are the Technical Architect Agent for an IT services company.
...
def save_client_detailed_requirement(report, client_account, requirement_text, uploaded_file_name=""):
    ...
    row = {
        ...
        "module_breakdown": "See Technical Architect Agent plan",
        ...
        "approval_status": "Technical Architect Agent Drafted - Manager Review Pending",
        "approved_by": "",
        "approved_at": "",
        "created_at": now,
        "updated_at": now,
    ...
    try:
        ...
        if mask.any():
            ...
            df.loc[mask, "last_updated_role"] = "Client"
            df.loc[mask, "last_updated_at"] = now
            df.loc[mask, "last_update_summary"] = "Client submitted detailed requirement. Technical Architect Agent created delivery plan for manager review."
            upsert_current_proposal_from_df(df, mask)
    except Exception:
        pass
    append_proposal_history(
        ...
        summary="Client submitted detailed requirement; Technical Architect Agent created delivery plan.",
    )
    return True, "Detailed requirement submitted. Technical Architect Agent has created a delivery plan and notified the Technical Architect Agent."
...
def get_operations_notification_recipients(project_id=""):
    """Return Technical Architect Agent user IDs that should receive delivery/update alerts."""
    recipients = []
...
def analyze_employee_update_with_llm(alloc, user, progress_status, progress_percent, hurdles, support_needed, notes):
    ...
    Employees may describe risk in many styles and languages. The Technical Architect Agent
    ...
    prompt = f"""
You are LifecycleDesk AI's Technical Architect Agent.
Read the employee's weekly project update and decide whether the Technical Architect Agent needs to take action.
...
- recommended_action: one concise instruction for the Technical Architect Agent
...
def save_employee_project_update(alloc, user, progress_status, progress_percent, hurdles, support_needed, notes):
    ...
    message = (
        f"Technical Architect Agent alert: {actor_name(user)} submitted a weekly update for project {project_id}, "
    ...
    if recipients:
        return True, "Thanks. Your weekly update/request was saved. The Technical Architect Agent has been notified and will connect with you soon if follow-up is needed."
...
def generate_weekly_update_if_needed(project, delivery_plan, allocations):
    ...
    fallback = (
        ...
        "At the moment, there are no major blockers recorded. If you have any change in priority, scope, branding, or integrations, please share it with the Technical Architect Agent through this portal."
    )
    if LIGHTWEIGHT_FAST_MODE and not USE_LLM_FOR_LONG_DOCUMENTS:
        ...
        row = {
            ...
            "client_action_needed": "Share changes/blockers with Technical Architect Agent if any",
...
def approve_delivery_plan_and_allocate(project, plan, user, manager_plan_text, selected_rows):
    ...
    plan.update({
        ...
        "approval_status": "Approved by Technical Architect Agent",
    ...
    try:
        ...
        if mask.any():
            ...
            df.loc[mask, "last_updated_at"] = now
            df.loc[mask, "last_update_summary"] = "Technical Architect Agent approved delivery plan and allocated project team."
            for role_key, column in PROPOSAL_NOTIFICATION_COLUMNS.items():
                df.loc[mask, column] = "No"
                df.loc[mask, PROPOSAL_SEEN_AT_COLUMNS[role_key]] = ""
            upsert_current_proposal_from_df(df, mask)
    ...
    append_proposal_history(
        ...
        summary=f"Technical Architect Agent approved delivery plan and allocated {len(selected_rows)} employee(s).",
    )
...
def save_client_operations_message(project, report, client_account, message_text, sender_type="Client", sender_user=None):
    ...
    if sender_type == "Technical Architect":
        ...
        sender_name = actor_name(sender_user or {}) or "Technical Architect Agent"
        seen_by_client = "No"
...
def save_client_message_to_operations(project, report, client_account, message_text):
    """Send a client portal message to the Technical Architect Agent and keep conversation history."""
    message_text = safe_text(message_text).strip()
    if not message_text:
        return False, "Please type a message before sending."
    ...
    return True, "Your message has been sent to the Technical Architect Agent."
...
def render_client_delivery_section(report, client_account):
    ...
    if status in ["Approved by Technical Architect Agent", "Approved by Operations Manager"]:
...
def render_operations_delivery_workspace(user, data, focus_proposal_id=None):
    """Clean Technical Architect Agent delivery workspace.
    ...
    st.markdown(
        ...
        <div class="dd-section-card dd-dashboard-shell">
            ...
            <div class="dd-section-subtitle">
                Select one accepted project, review the client requirement, check the Technical Architect Agent conclusion,
    ...
    if merged.empty:
        st.info("No delivery plan found for this accepted project yet. Once the client submits the detailed requirement, the Technical Architect Agent analysis and allocation controls will appear here.")
        return
    ...
    with st.expander("Technical Architect Agent conclusion", expanded=False):
        if agent_conclusion:
            ...
            st.info("Technical Architect Agent analysis is not available yet. It is generated after the client submits a detailed requirement.")
    ...
    with st.expander("Delivery plan and team allocation", expanded=False):
        ...
        manager_plan = st.text_area(
            "Technical Architect Agent delivery plan",
        ...
        button_text = "Approve delivery plan and notify selected employees" if safe_text(plan.get("approval_status")) not in ["Approved by Technical Architect Agent", "Approved by Operations Manager"] else "Update team allocation and notify selected employees"
...
def render_employee_project_workbench(user):
    ...
    a time, the client requirement, the Technical Architect Agent conclusion, and the
    ...
    if st.session_state.get(ack_key, False):
        st.success("Your update/request has been sent to the Technical Architect Agent.")
    ...
    # Technical Architect Agent conclusion, and the required weekly update form.
